...
public class Configuration {
  ...
  /**
   * mapper的注册集合，这个对象主要持有一个config对象和类型与MapperProxyFactory的Map集合
   */
  ...
  public Configuration() {
    ...
    //和FifoCache缓冲用到的设计模式一致，采用委派设计模式(这里翻译使用的是装饰者的设计模式
    //这个实现主要通过最近使用的思路实现缓冲的个数恒定，默认个数也是1024个
    //这里用到LinkedHashMap实现。详细解释可以查看具体实现类
    typeAliasRegistry.registerAlias("LRU", LruCache.class);

    //软引用的缓冲，JVM中对于引用有四种形式，强引用(默认)、软应用、弱引用、虚引用
    //引用类型	        用途	        被回收时间	        生存时间
  ...
  /**
   * 严格的HashMap
   * 主要用于实现对于已创建的key-value，不能在插入相同的key值。
   * 如果通过key获取value，如果key不存在，则也会抛出异常
   * 使用到这个类的变量主要包括:
   *   {@linkplain #caches} {@linkplain #resultMaps}{@linkplain #parameterMaps}
   *   {@linkplain #keyGenerators}{@linkplain #sqlFragments}
   * @param <V>
   */
